...
class w {
  ...
  get(e) {
    if (e >= this.size) throw new RangeError("Bit index out of range");
    const s = y(e), o = l(e);
...
function E(r) {
  ...
  for (let t = 0; t < s; t++) e.push([r[t * 3], r[t * 3 + 1], r[t * 3 + 2]]);
  ...
  const n = new w(s), f = [], p = (t) => {
    ...
    for (; c.length > 0; ) {
      const a = c.pop();
      n.get(a) || (n.set(a, true), f.push(e[a]), e[a].forEach((i) => {
        const u = o.get(i);
        u && u.forEach((g) => {
          n.get(g) || c.push(g);
        });
